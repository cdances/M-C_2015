...
The RDFMG version COBRA-TF is referred to as CTF.
...
to no verificaiton work previously. 
...
initial verification of the original verion of code as well as the residual
...
will be the beginning of a verification manual, integrating this verificaiton problem
...
consistencey / convergence, and order of accuracy \cite{VV_Book}. For this work
the a Richardson Extrapolation will be used to check for convergence and order
...
The verification problem is defined as a single horizontal channel problem the
...
fluctuations may occur due to coding mistakes or numerical noise. The channel
...
of the transient was defined to be quadruple the time needed for the liquid at
...
%as shown in figure \ref{fig:Inlet_vel}.
 ...
 this problem and has considerably less fluctation in the inlet velocity. The
 hdf5 output files allow for a high level of precision of the code, reducing
 round off error in the output. 

\subsection{Code Convergence}

The current version of CTF uses global code convergance criteria that are
...
\begin{figure}[!h]
	...
	\caption{Sumnation of the residuals for the residual version of CTF}
...
each additinal refinement level, the amount of computational resources doubles.
For this analysis, a significant amount of infromation was added to the hdf5
...
function of temporal and spatial step size respecitvely. 

\begin{figure}[!h]
	\centering
	\includegraphics[width=0.70\textwidth]{images/Temporal_Study/Difference_h}
...
to the small fluctations in the velocity present in the original version of the
code. While only the enthalpy is shown here, the mass flow rate and the density
showed similar trends.

\begin{figure}[!h]
	\centering
	\includegraphics[width=0.70\textwidth]{images/Spatial_Study/Difference_h}
	\caption{Difference Between Succssessive Temporal Refinements}
...
the modified equation analysis. This can be considred to be the exponent on
...
study. The refinement factor, $R$, has the constnat value of 2 for both the
...
the aysmptotic range, but also approaches an order of accuracy of 1.0 with
decreasing mesh size. 

\begin{figure}[!h]
	\centering
	\includegraphics[width=0.70\textwidth]{images/Temporal_Study/Order_Of_Accuracy_Summary}
	\caption{Temporal Order of Accuracy}
	\label{fig:Temporal:OOA}
\end{figure}

The slight differences between the orginal version of CTF and the residual
...
order of accuracies very close the the values obtained through the modifed
equation analysis.

\begin{figure}[!h]
	\centering
	\includegraphics[width=0.70\textwidth]{images/Spatial_Study/Order_Of_Accuracy_Summary}
...
was shown to be a good verificaiton problem through its isolation of the order
...
might be comparing the resuls with CFL number of 1 to the known soluions of the
code. This would be a less rigourous study, but would be a quick verificaiton
